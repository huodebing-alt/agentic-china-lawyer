...
def lookup(law_name: str, article: int) -> Dict[str, Any]:
    ...
    return {
        ...
        "warning": f"本地法规库未收录《{law_name}》。" + _placeholder_warning(),
    }


def search(query: str = "民法典", top_k: int = 5) -> Dict[str, Any]:
    """关键词检索全部条款（含命中次数评分）。"""
    results = []
...
def _main():
    ...
    if "--selftest" in sys.argv:
        ...
        import types
        sys.exit(run_selftest(sys.modules[__name__], expected=["lookup", "search", "list_laws", "prepare_status"]))
